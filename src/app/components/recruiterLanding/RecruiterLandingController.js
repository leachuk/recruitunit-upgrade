import template from './recruiterLanding.html';
...
class RecruiterLandingController {
  ...
  showFormDetailDialog($event, item){
    console.log("in showFormDetailDialog");
    console.log("item:");
    console.log(item);
    var id, isPass, isPartialPass = "";
    if (item.model == "RecruitUnitJobDescription"){
      id = item._id;
      isPass = "";
      isPartialPass = "";
		} else {
			id = item.id;
			isPass = item.isPass;
			isPartialPass = item.isPartialPass;
		}
...
export default {
  controller: RecruiterLandingController,
  controllerAs: 'recruiterLanding',
  template: template,
  $canActivate: function($nextInstruction, recruitUnitUtil, jwtHelper, loomApi, globals, lodash, recruitUnitConfig){
    "ngInject";
    console.log("in RecruiterLandingController canActivate");
    //console.log(recruitUnitConfig);
